...
process.options = cms.untracked.PSet(
    wantSummary=cms.untracked.bool(True), allowUnscheduled=cms.untracked.bool(False)
)
...
process.centralGen = cms.EDFilter(
    'CandPtrSelector',
    src=cms.InputTag('genParticlesForMETAllVisible'),
    cut=cms.string('abs(eta) < 2.4'),
)
process.barrelGen = cms.EDFilter(
    'CandPtrSelector',
    src=cms.InputTag('genParticlesForMETAllVisible'),
    cut=cms.string('abs(eta) < 1.5'),
)
...
def monitorPerf(
    ...
    makeMET=True,
    makeCentralMET=True,
    makeInputMultiplicities=False,
    makeOutputMultiplicities=False,
    saveCands=False,
):
    def _add(name, what):
    ...
    if type(tag) != str and len(tag) > 1:
        _add(
            'merged' + label,
            cms.EDProducer(
                'L1TPFCandMerger', src=cms.VInputTag(cms.InputTag(x) for x in tag)
            ),
        )
        tag = 'merged' + label
    if makeResp:
        ...
        if makeRespSplit:
            setattr(
                ...
                label + 'Charged',
                cms.VInputTag(cms.InputTag(tag)),
            )
            setattr(
                process.ntuple.objects, label + 'Charged_sel', cms.string('charge != 0')
            )
            setattr(
                process.ntuple.objects,
                label + 'Photon',
            ...
            setattr(
                process.ntuple.objects, label + 'Photon_sel', cms.string('pdgId == 22')
            )
            setattr(
                process.ntuple.objects,
                label + 'Neutral',
            ...
            setattr(
                process.ntuple.objects, label + 'Neutral_sel', cms.string('charge == 0')
            )
            setattr(
                process.ntuple.objects,
                label + 'NeutralHad',
    ...
    if makeMET:
        ...
        if makeCentralMET:
            ...
            _add(
                'met' + label + 'Central',
                pfMet.clone(src='central' + label, calculateSignificance=False),
            )
            setattr(
                process.l1pfmetCentralTable.mets,
                label,
                cms.InputTag('met' + label + 'Central'),
            )
...
process.l1pfmetTable = cms.EDProducer(
    'L1PFMetTableProducer',
    genMet=cms.InputTag('genMetTrue'),
    flavour=cms.string(''),
    mets=cms.PSet(),
)
process.l1pfmetCentralTable = process.l1pfmetTable.clone(
    genMet='genMetCentralTrue', flavour='Central'
)
...
process.p = cms.Path(
    ...
    + process.l1pfmetTable
    + process.l1pfmetCentralTable
)
process.p.associate(process.extraPFStuff)
process.TFileService = cms.Service(
    'TFileService', fileName=cms.string('perfTuple.root')
)
...
process.outnano = cms.OutputModule(
    'NanoAODOutputModule',
...
if True:
    process.genInAcceptance = cms.EDFilter(
        'GenParticleSelector',
        ...
        cut=cms.string(
            ...
            + 'abs(pdgId) == 22 && pt > 1 || '
            + 'charge == 0 && pt > 1 || '
            + 'charge != 0 && abs(eta) > 2.5 && pt > 2) '
        ),  # tracks below pT 2 bend by more than 0.4,
    )
    ...
    process.ntuple.objects.ChGenAcc_sel = cms.string(
        '(abs(eta) < 2.5 && pt > 2 && charge != 0)'
    )
...
def addMult():
    for D in ['Barrel', 'HF', 'HGCal', 'HGCalNoTK']:
        monitorPerf(
            ...
            makeJets=False,
            makeMET=False,
            makeCentralMET=False,
            makeInputMultiplicities='CTL1',
        )
        monitorPerf(
            'L1%sEmCalo' % D,
            'l1tLayer1%s:EmCalo' % D,
            makeResp=False,
            makeRespSplit=False,
            makeJets=False,
            makeMET=False,
...
def addCHS():
    process.l1PuppiCharged = cms.EDFilter(
        'L1TPFCandSelector',
        src=cms.InputTag('l1tLayer1:Puppi'),
        cut=cms.string('charge != 0'),
    )
    process.l1PFNeutral = cms.EDFilter(
        'L1TPFCandSelector',
...
def addCalib():
    ...
    process.l1tPFClustersFromL1EGClustersRaw = (
        process.l1tPFClustersFromL1EGClusters.clone(corrector='')
    )
    process.l1tPFClustersFromHGC3DClustersRaw = (
        process.l1tPFClustersFromHGC3DClusters.clone(corrector='')
    )
    process.l1tPFClustersFromHGC3DClustersEMRaw = (
        process.l1tPFClustersFromHGC3DClustersEM.clone(corrector='')
    )
    process.extraPFStuff.add(
        process.l1tPFClustersFromL1EGClustersRaw,
        process.l1tPFClustersFromHGC3DClustersRaw,
    ...
    process.ntuple.objects.L1RawBarrelEcal = cms.VInputTag(
        'l1tPFClustersFromL1EGClustersRaw'
    )
    process.ntuple.objects.L1RawBarrelCalo = cms.VInputTag(
        'l1tPFClustersFromCombinedCaloHCal:uncalibrated'
    )
    process.ntuple.objects.L1RawBarrelCaloEM = cms.VInputTag(
        'l1tPFClustersFromCombinedCaloHCal:emUncalibrated'
    )
    process.ntuple.objects.L1RawHGCal = cms.VInputTag(
        'l1tPFClustersFromHGC3DClustersRaw'
    )
    process.ntuple.objects.L1RawHGCalEM = cms.VInputTag(
        'l1tPFClustersFromHGC3DClustersEMRaw'
    )
    process.ntuple.objects.L1RawHFCalo = cms.VInputTag(
        'l1tPFClustersFromCombinedCaloHF:uncalibrated'
    )
    process.ntuple.objects.L1BarrelEcal = cms.VInputTag('l1tPFClustersFromL1EGClusters')
    process.ntuple.objects.L1BarrelCalo = cms.VInputTag(
        'l1tPFClustersFromCombinedCaloHCal:calibrated'
    )
    process.ntuple.objects.L1HGCal = cms.VInputTag('l1tPFClustersFromHGC3DClusters')
    process.ntuple.objects.L1HFCalo = cms.VInputTag(
        'l1tPFClustersFromCombinedCaloHF:calibrated'
    )
    process.ntuple.objects.L1HGCalEM = cms.VInputTag(
        'l1tPFClustersFromHGC3DClustersEM',
    )
...
def addSeededConeJets():
    ...
    process.l1pfjetTable.jets.scPuppiCorr = cms.InputTag(
        'l1tSCPFL1PuppiCorrectedEmulator'
    )
    process.l1pfmetTable.mets.scPuppiCorrMHT = cms.InputTag(
        'l1tSCPFL1PuppiCorrectedEmulatorMHT'
    )


def addPhase1Jets():
    process.extraPFStuff.add(
        process.l1tPhase1JetProducer9x9,
        process.l1tPhase1JetCalibrator9x9,
        process.l1tPhase1JetSumsProducer9x9,
    )
    process.extraPFStuff.add(
        process.l1tPhase1JetProducer9x9trimmed,
        process.l1tPhase1JetCalibrator9x9trimmed,
        process.l1tPhase1JetSumsProducer9x9trimmed,
    )
    process.l1pfjetTable.jets.phase19x9Puppi = cms.InputTag(
        'l1tPhase1JetProducer9x9', 'UncalibratedPhase1L1TJetFromPfCandidates'
    )
    process.l1pfjetTable.jets.phase19x9PuppiCorr = cms.InputTag(
        'l1tPhase1JetCalibrator9x9', 'Phase1L1TJetFromPfCandidates'
    )
    process.l1pfjetTable.jets.phase19x9trimmedPuppi = cms.InputTag(
        'l1tPhase1JetProducer9x9trimmed', 'UncalibratedPhase1L1TJetFromPfCandidates'
    )
    process.l1pfjetTable.jets.phase19x9trimmedPuppiCorr = cms.InputTag(
        'l1tPhase1JetCalibrator9x9trimmed', 'Phase1L1TJetFromPfCandidates'
    )
    process.l1pfmetTable.mets.scPuppiCorrMHT = cms.InputTag(
        'l1tSCPFL1PuppiCorrectedEmulatorMHT'
    )
...
def addCaloJets():
    ...
    process.l1pfjetTable.jets.RefCaloJets = cms.InputTag(
        'l1tCaloJet', 'L1CaloJetCollectionBXV'
    )
...
def addTkJets():
    ...
    process.l1pfmetTable.mets.RefL1TrackerEtMiss = cms.InputTag(
        'L1TrackerEmuEtMiss', 'L1TrackerEmuEtMiss'
    )
    process.l1pfmetTable.mets.RefL1TrackerHTMiss = cms.InputTag(
        'L1TrackerEmuHTMiss', 'L1TrackerEmuHTMiss'
    )
...
def addJetConstituents(N):
    for i in range(
        N
    ):  # save a max of N daughters (unfortunately 2D arrays are not yet supported in the NanoAOD output module)
        for var in 'pt', 'eta', 'phi', 'mass', 'pdgId':
            setattr(
                process.l1pfjetTable.moreVariables,
                'dau%d_%s' % (i, var),
                cms.string(
                    '? numberOfDaughters() > %d ? daughter(%d).%s : -1' % (i, i, var)
                ),
            )
        setattr(
            process.l1pfjetTable.moreVariables,
            'dau%d_%s' % (i, 'vz'),
            cms.string(
                '? numberOfDaughters() > %d ? daughter(%d).%s : -1' % (i, i, 'vertex.Z')
            ),
        )
...
def addGenJetFlavourTable():
    ...
    process.genJetFlavourTable = cms.EDProducer(
        'GenJetFlavourTableProducer',
        name=cms.string('GenJets'),
        src=process.l1pfjetTable.jets.Gen,
        cut=cms.string(
            f'{process.l1pfjetTable.commonSel.value()} && {process.l1pfjetTable.jets.Gen_sel.value()}'
        ),
...
def addTkPtCut(ptCut):
    ...
    process.l1tLayer1TkPt3 = cms.EDProducer(
    ...
    process.extraPFStuff.add(
        process.l1tLayer1BarrelTkPt3,
        process.l1tLayer1HGCalTkPt3,
        process.l1tLayer1TkPt3,
    )
    ...
    for pdgId in pdgs:
        if pdgId == 13:
            ...
            process.genMuExtTable = genLepTableExt.clone(
                cut=process.genMuTable.cut, name=process.genMuTable.name
            )
...
def addHGCalTPs():
    process.hgcClusterTable = cms.EDProducer(
        'SimpleCandidateFlatTableProducer',
        src=cms.InputTag(
    ...
    process.hgcClusterExtTable = cms.EDProducer(
    ...
    for w in opts:
        ...
        for pdgId in pdgs:
            if pdgId == 13:
                ...
                muTable.variables.quality = Var(
                    '? muon.isNonnull ? muon.hwQual : -1', int, doc='Quality'
                )
            elif pdgId == 11:
                ...
                elTable.variables.hwEmID = Var(
                    '? pfCluster.isNonnull ? pfCluster.hwEmID : -1', int, doc='Quality'
                )
                elTable.variables.pfEmID = Var(
                    '? pfCluster.isNonnull ? pfCluster.egVsPionMVAOut : -1',
                    float,
                    precision=8,
                )
                elTable.variables.pfPuID = Var(
                    '? pfCluster.isNonnull ? pfCluster.egVsPUMVAOut : -1',
                    float,
                    precision=8,
                )
                elTable.variables.clPt = Var(
                    '? pfCluster.isNonnull ? pfCluster.pt : -1', float, precision=8
                )
                elTable.variables.clEmEt = Var(
                    '? pfCluster.isNonnull ? pfCluster.emEt : -1', float, precision=8
                )
            elif pdgId == 22:
...
def addStaEG(postfix=''):
    ...
    staEgEBEmuTable = getStaEgTables(
        'EB', postfix, 'l1tPhase2L1CaloEGammaEmulator:GCTEGammas'
    )
    process.EGStaEBEmuTable = staEgEBEmuTable
    process.extraPFStuff.add(staEgEBEmuTable)

    staEgEEEmuTable = getStaEgTables('EE', postfix, 'l1tLayer1EG:L1EgEE')
    process.EGStaEETable = staEgEEEmuTable
...
def addTkEG(doL1=False, doL2=True, postfix=''):
    ...
    if doL1:
        for w in 'EB', 'EE':
            tkEmTable, tkEleTable = getTkEgTables(
                w,
                postfix,
                f'l1tLayer1EG{postfix}:L1TkEm{w}',
                f'l1tLayer1EG{postfix}:L1TkEle{w}',
            )
    ...
    if doL2:
        tkEmTable, tkEleTable = getTkEgTables(
            'L2', postfix, 'l1tLayer2EG:L1CtTkEm', 'l1tLayer2EG:L1CtTkElectron'
        )
...
def addEGCrystalClusters() -> None:
    def getCrystalClustersTable(nameSrcDict: dict[str, str]) -> cms.EDProducer:
        CrystalClustersTable = cms.EDProducer(
            'SimpleCandidateFlatTableProducer',
    ...
    nameSrcDictList = [
        {
            'name': 'CaloEGammaCrystalClustersRCT',
            'src': 'l1tPhase2L1CaloEGammaEmulator:RCTClusters',
        },
        {
            'name': 'CaloEGammaCrystalClustersGCT',
            'src': 'l1tPhase2L1CaloEGammaEmulator:GCTClusters',
        },
    ]
...
def oldInputs_11_1_6():
    process.l1tTowerCalibration.l1CaloTowers = cms.InputTag(
        'L1EGammaClusterEmuProducer', ''
    )
    process.l1tTowerCalibration.L1HgcalTowersInputTag = cms.InputTag(
        'hgcalTowerProducer', 'HGCalTowerProcessor'
    )
    process.l1tPFClustersFromL1EGClusters.src = cms.InputTag(
        'L1EGammaClusterEmuProducer',
    )
    process.l1tPFClustersFromCombinedCaloHCal.phase2barrelCaloTowers = [
        cms.InputTag(
            'L1EGammaClusterEmuProducer',
        )
    ]
    process.l1tPFClustersFromHGC3DClusters.src = cms.InputTag(
        'hgcalBackEndLayer2Producer', 'HGCalBackendLayer2Processor3DClustering'
    )
    process.l1tPFClustersFromCombinedCaloHF.hcalCandidates = [
        cms.InputTag(
            'hgcalBackEndLayer2Producer', 'HGCalBackendLayer2Processor3DClustering'
        )
    ]
    process.l1tPFTracksFromL1Tracks.L1TrackTag = cms.InputTag(
        'TTTracksFromTrackletEmulation', 'Level1TTTracks'
    )
...
def oldInputs_12_3_X():
    process.l1tTowerCalibration.l1CaloTowers = cms.InputTag(
        'L1EGammaClusterEmuProducer', 'L1CaloTowerCollection'
    )
    process.l1tTowerCalibration.L1HgcalTowersInputTag = cms.InputTag(
        'hgcalTowerProducer', 'HGCalTowerProcessor'
    )
    ...
    process.l1tPFClustersFromCombinedCaloHCal.phase2barrelCaloTowers = [
        cms.InputTag('L1EGammaClusterEmuProducer', 'L1CaloTowerCollection')
    ]
    process.l1tPFClustersFromHGC3DClusters.src = cms.InputTag(
        'hgcalBackEndLayer2Producer', 'HGCalBackendLayer2Processor3DClustering'
    )
...
if False:
    ...
    process.source.fileNames = [
        '/store/cmst3/group/l1tr/gpetrucc/12_3_X/NewInputs110X/220322/TTbar_PU200/inputs110X_%d.root'
        % i
        for i in (1,)
    ]
...
def saveGenCands():
    process.gencandTable = cms.EDProducer(
        'L1PFCandTableProducer',
        commonSel=cms.string('pt > 0.0'),
        cands=cms.PSet(Gen=cms.InputTag('genParticlesForMETAllVisible')),
        moreVariables=cms.PSet(pdgId=cms.string('pdgId'), charge=cms.string('charge')),
    )
